...
def test_reset_scale_and_zero_point():
    num_samples = 1000000
    distinct_distributions_list = ['normal', 'uniform', 'laplacian', 'von_mises']
    methods = ['sym', 'SAWB', "heuristic", "asym"]
    bits = [8, 4, 2]
    for method in methods:
        for b in bits:
            for dist in distinct_distributions_list:
                ...
                if method == "asym" : 
                    quantized_tensor = linear_quantize(test_tensor, scale, zero_point, b, signed = False)
                else : 
                ...
                assert torch.allclose(test_tensor, dequantized_tensor, atol=atol) or mse<0.05 , f"failed for {b}bit quantization with {method} method!"
    print('* Test passed.')
...
def test_quantized_linear_function(weight_N_bits=4, act_N_bits=4, method='sym', bias=True):
    ...
    input_scale, zero_point = reset_scale_unsigned(real_input, act_N_bits)
...
def test_quantized_linear_module(weight_N_bits=8, act_N_bits=8, method='sym', bias=True):
    ...
    real_input = torch.rand(128, 1000)
    m_q.input_scale.data, _ = reset_scale_unsigned(real_input, act_N_bits)
    m_q.method = 'normal'
...
def plot_real_dequantized_histogram(x,N_bits=4, log=True):
    ...
    for j in range(4):
        ...
        if method == "asym" : 
            x_int = linear_quantize(x, scale, zero_point, N_bits, signed= False)
        else : 
